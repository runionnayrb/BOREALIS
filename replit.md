...
- **Settings Management**: Full CRUD for all entities (scenes, acts, departments, locations, artists, technicians, report template) including drag-and-drop artist reordering. Artist profile photos support client-side crop adjustment with circular preview, zoom control (1x-3x), and position adjustment before upload. Technicians support per-department ordering with optimistic UI updates - each department maintains independent sortOrder allowing technicians to appear in different positions across departments. Drag-and-drop reordering uses optimistic updates for instant feedback (UI updates immediately, API syncs in background, automatic rollback on failure).
